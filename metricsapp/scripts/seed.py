...
def import_data():
    ...
    metricsapp_dailyuserstorythroughput_1.endpoint = 'http://192.168.30.233:7478/db/data/transaction/commit'
    ...
    metricsapp_justintimedevelopment_1.endpoint = 'http://192.168.30.233:7478/db/data/transaction/commit'
    ...
    metricsapp_forgottenones_1.endpoint = 'http://192.168.30.233:7478/db/data/transaction/commit'
    ...
    metricsapp_neverendingstory_1.endpoint = 'http://192.168.30.233:7478/db/data/transaction/commit'
    ...
    metricsapp_speedyprs_1.endpoint = 'http://192.168.30.233:7478/db/data/transaction/commit'
    ...
    metricsapp_personal_code_ownership_1.query = 'MATCH (fc:GithubFileChange)-[:files]-(c:NonMergeCommit)-[]-(u:GithubUser) WHERE has(fc.filename) AND fc.filename<>"" AND u.role <> "org" AND u.team = "{team}" MATCH c-[:milestone]-(m:GithubMilestone) WHERE m.title="{sprint}" WITH fc, u ORDER BY u.login WITH fc.filename as Filename, collect(DISTINCT u) as Authors, count(fc) as Edits WITH Filename, Authors, Edits, length(Authors) as AuthorAmount WHERE Edits > 8 AND AuthorAmount < 3 RETURN Filename,  AuthorAmount, Authors, Edits ORDER BY AuthorAmount ASC, Edits DESC'
    metricsapp_personal_code_ownership_1.endpoint = 'http://192.168.30.233:7478/db/data/transaction/commit'
    ...
    metricsapp_monster_stories_1.explanation = 'User stories should be small enough to get a quick overview of the work to be done, but should contain enough information to be estimatable by the developers. It is sometimes stated that user stories should fit on an index card. The issues below have double the average length of issues in the selected sprint or have double the amount of checkboxes of other stories. Both of these can be an indicator that a user story is too large and could be split. Smaller user stories can improve the ability to plan as they are less likely to deviate from the estimation and sprint backlogs can be filled on a more granular basis.\r\n'
    ...
    metricsapp_monster_stories_1.endpoint = 'http://192.168.30.233:7478/db/data/transaction/commit'
    ...
    metricsapp_untested_complexity_1.endpoint = 'http://192.168.30.233:7478/db/data/transaction/commit'
    ...
    metricsapp_tutor_scores_1.endpoint = 'http://192.168.30.233:7478/db/data/transaction/commit'
    ...
    metricsapp_percent_metric_1.endpoint = 'http://192.168.30.233:7478/db/data/transaction/commit'
